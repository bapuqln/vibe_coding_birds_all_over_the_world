...
# V33 — AI Bird Guide (Implementation Plan)

## Goal

Introduce a multi-layer knowledge system where clicking a bird generates a short, kid-friendly explanation. The system queries local static data first, then local cache, then a remote AI provider. Results are cached so each bird triggers AI at most once.

## Architecture Layers

### Layer 1: PromptTemplate Registry (`src/features/prompt-templates.ts`)

Define all prompt templates as data. No hardcoded prompt strings anywhere else.

Templates:

- `bird-explain`: System prompt for kid-friendly bird educator + user prompt with `{{birdName}}`, `{{habitat}}`, `{{funFact}}`, `{{diet}}`, `{{region}}` variables.

Implementation: `getTemplate(id)` returns template object. `renderTemplate(template, vars)` interpolates variables.

### Layer 2: AIProvider Abstraction (`src/features/ai-provider.ts`)

Abstract interface for AI generation. Concrete implementation: `OpenAIProvider`.

- `isAvailable()`: checks `VITE_OPENAI_API_KEY`.
- `generate(system, user)`: calls OpenAI chat completions, returns parsed JSON `{ text, textZh }`.
- Returns `null` on failure (network, auth, parse errors).

### Layer 3: KnowledgeCache (`src/features/knowledge-cache.ts`)

localStorage-backed cache.

- Key format: `bird-guide-cache-{birdId}`.
- Value: `{ text, textZh, timestamp }`.
- `get(birdId)` / `set(birdId, result)`.
- LRU eviction at 200 entries.

### Layer 4: KnowledgeService (`src/features/KnowledgeService.ts`)

Orchestrator. Single public function: `queryBirdExplanation(birdId): Promise<KnowledgeResult>`.

Flow:

1. Build L1 answer from bird data (name, habitat, fun fact, diet → template rendering).
2. Check L2 cache. If hit, return `{ text, textZh, source: "cache" }`.
3. If AI provider available, render prompt template, call provider, cache result in L2, return `{ text, textZh, source: "ai" }`.
4. If AI unavailable or fails, return L1 result with `source: "static"`.

### Layer 5: TTSService (`src/features/tts-service.ts`)

Thin wrapper around Web Speech API.

- `speak(text, lang)`: creates utterance, rate 0.9, speaks.
- `stop()`: cancels current speech.
- `isAvailable()`: checks `speechSynthesis` support.

### Layer 6: Store Integration (`src/store.ts`)

New state:

- `birdExplanation: { text: string; textZh: string; source: string } | null`
- `birdExplanationLoading: boolean`
- `ttsStatus: "idle" | "speaking" | "unavailable"`

New actions:

- `requestBirdExplanation(birdId: string)`: async, calls KnowledgeService, sets result.
- `clearBirdExplanation()`: resets.
- `speakExplanation()`: triggers TTS.
- `stopSpeaking()`: cancels TTS.

### Layer 7: UI Changes

**BirdInfoCard.tsx**: Add "Tell me about this bird!" button. On click, dispatches `requestBirdExplanation`.

**AIBirdGuidePanel.tsx**: Shows explanation with typing animation when `birdExplanation` is set. Shows TTS button after typing completes. Shows loading spinner during AI call. Shows source badge (static/cache/AI).

## Phase Order

1. Types (`types.ts`)
2. PromptTemplate registry
3. AIProvider abstraction
4. KnowledgeCache
5. KnowledgeService (orchestrator)
6. TTSService
7. Store state + actions
8. UI: BirdInfoCard button
9. UI: AIBirdGuidePanel upgrade
...
| Component                      | Changes                                                    |
| ------------------------------ | ---------------------------------------------------------- |
| `types.ts`                     | Add `KnowledgeResult`, `PromptTemplate`, `TTSStatus` types |
| `store.ts`                     | Add explanation state, loading, TTS status, actions        |
| `BirdInfoCard.tsx`             | Add "Tell me!" button                                      |
| `AIBirdGuidePanel.tsx`         | Show explanation, TTS, source badge                        |
| `features/BirdGuideService.ts` | Refactor to use KnowledgeService                           |
| `features/PromptBuilder.ts`    | Replace with PromptTemplate system                         |

## New Files

| File                               | Purpose                               |
| ---------------------------------- | ------------------------------------- |
| `src/features/prompt-templates.ts` | PromptTemplate registry               |
| `src/features/ai-provider.ts`      | AIProvider interface + OpenAIProvider |
| `src/features/knowledge-cache.ts`  | localStorage cache with LRU           |
| `src/features/KnowledgeService.ts` | Multi-layer orchestrator              |
| `src/features/tts-service.ts`      | Web Speech API wrapper                |

---

# V33 Upgrade Plan — AI Bird Guide + Mode-Driven HUD Refactor
...
┌─────────────────────────────────────────────────────────────────┐
│                        UI Layer (React)                         │
│  ModeSelector → ModeGate (visibility filter) → Component Panels│
│         │                                      │                │
│         ▼                                      ▼                │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │              Zustand Store (state + actions)             │   │
│  │  appMode, selectedBirdId, birdExplanation, ttsStatus    │   │
│  └────────────────────────┬────────────────────────────────┘   │
└───────────────────────────┼─────────────────────────────────────┘
                            │ dynamic import()
┌───────────────────────────┼─────────────────────────────────────┐
│                    Service Layer                                 │
│  KnowledgeService → L2 Cache → L3 AIProvider → L1 Static       │
...
```
User clicks mode    → setAppMode(mode) → panels re-evaluate visibility
User clicks bird    → setSelectedBird(id) → BirdInfoCard renders
User clicks "Tell!" → requestBirdExplanation(id) → KnowledgeService
                      → L2 cache HIT → return cached
                      → L2 MISS → L3 AI → cache result → return
                      → L3 FAIL → L1 static → return
User clicks TTS     → speakExplanation() → TTSService.speak()
```

## Performance Strategy

1. Lazy loading: Services loaded via `import()` only when needed.
2. Cache-first: L2 checked before network. Each bird triggers AI max once.
3. Mode-gated rendering: Components not in current mode return `null`.
4. Stable selectors: Zustand shallow equality prevents re-renders.
5. Globe isolation: Canvas independent of panel state changes.

## Offline Fallback Strategy

1. No API key → skip L3, use L1 static.
2. Network error → L1 static fallback.
3. L2 cache persists across sessions (localStorage).
4. Source badge shows data origin (Local/Cached/AI).
5. No error modals — graceful degradation.

## ModeGate Component

```typescript
function ModeGate({ modes, children }: { modes: AppMode[]; children: React.ReactNode }) {
...
1. Types: Replace `UIMode` with `AppMode`
2. Store: Replace `uiMode`/`setUIMode` with `appMode`/`setAppMode`
3. ModeGate component
4. App.tsx: Wrap all panels in ModeGate
5. MainModePanel: Update mode definitions
6. KnowledgeService: Harden with error boundaries
7. AIProvider: Add timeout + error handling
8. AIBirdGuidePanel: Remove direct speech calls
